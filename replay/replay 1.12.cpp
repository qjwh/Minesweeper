...
int main(int argc, char **argv){
    ...
    {
        LL p = 0;
        #define linerd(x) ((x) = get_digit(line, p), p += 2)
        linerd(n), linerd(m), linerd(k);
        if(p < sz(line)){
            linerd(h);
            if(p < sz(line)){
                linerd(cx), linerd(cy);
    ...
    while(p < sz(replay)){
        char ch = replay[p];
        copt++;
        if(ch != 'w' && ch != 's' && ch != 'a' && ch != 'd' && ch != 'E' && ch != 'M' && ch != 't' && ch != 'o') copt--;
        if(ch == 't'){
            p += 1;
            LL x = get_digit(replay, p);
            p += 2;
            LL y = get_digit(replay, p);
        }
        if(iswin()) break;
        p++;
    }
