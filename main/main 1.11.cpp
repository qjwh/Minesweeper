...
void settings(){
    while(true){
        ...
        else if(opt == 3) api_mode = !api_mode;
        if(api_mode) challenge_mode = false;
    }
    export_settings();
}
...
LL get_digit(string replay, LL &x){
    LL ret = 0;
...
void solve(){
    ...
    while(true){
        ...
        if(!api_mode){
            ...
            ofstream fout("api.txt");
            fout << n << ' ' << m << ' ' << k << ' ' << h << ' ' << cx << ' ' << cy <<endl;
            rep(i, 1, n){
                rep(j, 1, m){
                    LL x;
                    if(st[i][j] == 0) x = -1;
                    else if(st[i][j] == 1) x = subget(i, j);
                    else x = 9;
                    fout << x << ' ';
                }
                fout <<endl;
        ...
        if(!api_mode){
            ...
            while(true){
                char ch;
                string line;
                if(system("dir opt.txt > NUL 2> NUL") == 0){
                    ifstream fin("opt.txt");
                    getline(fin, line);
                    fin.close();
                    system("del opt.txt > NUL 2> NUL");
